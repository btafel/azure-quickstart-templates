...
#start replica set
mongod --dbpath /var/lib/mongo/ --config /etc/mongod.conf --replSet $replSetName --shardsvr --logpath /var/log/mongodb/mongod.log --fork
...
mongod --dbpath /var/lib/mongo/ --replSet $replSetName --shardsvr --logpath /var/log/mongodb/mongod.log --fork --config /etc/mongod.conf
}
stop() {
pkill mongod
}
